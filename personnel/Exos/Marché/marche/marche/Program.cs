...
Product WhoMoreWaterMelon()
{
    int Quantity = 0;
    Product Producter = null;
    foreach (Product product in products)
    {
        if (product.ProductName == "Pastèques" && product.Quantity > Quantity)
        {
...
Product producer = WhoMoreWaterMelon();
int where(Product producer)
{
    return producer.Location;
}

string who(Product producer)
{
    return producer.Producer;
}

int howMany(Product producer)
{
...
Console.WriteLine($"il y a {CounterPeach()} vendeurs de pêches");
Console.WriteLine($"C'est {who(producer)} qui a le plus de pastèques (stand {where(producer)}, {howMany(producer)} pièces)");
